...
class EfficientNet(nn.Module):
    def __init__(self, comp_coef):
        super(EfficientNet, self).__init__()
        # model = EffNet.from_pretrained('efficientnet-b6')
        model = EffNet.from_pretrained('efficientnet-b'+str(comp_coef))
...
class EfficientDet(nn.Module):
    def __init__(self, num_anchors=9, num_classes=20, compound_coef=0):
        ...
        fpn_inp = fpn_inpsize_lis[compound_coef]

        self.conv3 = nn.Conv2d(fpn_inp[0], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv2d(fpn_inp[1], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv5 = nn.Conv2d(fpn_inp[2], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv6 = nn.Conv2d(fpn_inp[2], self.num_channels, kernel_size=3, stride=2, padding=1)
        ...
        self.backbone_net = EfficientNet(compound_coef)
